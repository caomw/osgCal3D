	...
	: Mesh(_modelData, _mesh)
{
	...
	// ^ no display lists since it is deprecated

	setUseVertexBufferObjects(true); // false is default

	modelData->getModel()->update(0.01);
	if (mesh->data->rigid)
	{
	...
	setTexCoordArray(0, mesh->data->texCoordBuffer, osg::Array::BIND_PER_VERTEX);
	if (mesh->data->normalBuffer.valid())
		setNormalArray(mesh->data->normalBuffer);
	if (mesh->data->tangentAndHandednessBuffer.valid())
	{
		...
		setTexCoordArray(1, mesh->data->tangentAndHandednessBuffer);
	}

 
	
	setTexCoordArray(2, mesh->data->weightBuffer, osg::Array::BIND_PER_VERTEX);
	...
	if (mesh->data->matrixIndexBuffer.valid())
	{
		...
		const GLbyte* end = m + mesh->data->matrixIndexBuffer->size() * 4;
			osg::ref_ptr<osg::Vec4sArray> vecarray = new osg::Vec4sArray();
	while (m < end)
		{
		osg::Vec4s v;
		v[0] = *m++;
		...
		v[2] = *m++;
		v[3] = *m++;
			vecarray->push_back(v);
			//*mi++ = *m++;
		}
		osg::ref_ptr<osg::MatrixfArray> matarray = new osg::MatrixfArray();
 
		setTexCoordArray(3, vecarray, osg::Array::BIND_PER_VERTEX);
...
const osg::StateSet* stateSet) const
{
	...
	Mesh::drawImplementation(renderInfo);
...
HardwareMesh::update()
{
	...
	osg::ref_ptr<osg::Vec3Array> temp = new osg::Vec3Array(vb, osg::CopyOp::DEEP_COPY_ALL);
	osg::Vec3f*        v = &temp->front();      /* dest vector */
